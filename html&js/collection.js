...
const _num =[1,4,5,8,10];
const _res =_num.reduce(( _prev, _curr)=>{
   return _prev + _curr;
});
...
const groupByplace = employees.reduce((result,employ)=>
{
    let place = employ.place;
    if(!result[place])
    {
        result[place]=[];
    }
    result[place].push(employ);
    return result;
},{});

console.log(groupByplace);


const groupBydesignation = employees.reduce((result,employ)=>
{
    ...
    result[designation].push(employ);
    return result;
},{});
